...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/btworeo/ultroid"),
        Button.url("• Support •", url="t.me/OreoSupportChat"),
    ],
]
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="OreO Help Menu", text=text, buttons=_main_help_menu
        )
...
async def _(event):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("oreo_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await event.edit(
            get_string("upd_5"),
            file="oreo_updates.txt",
            buttons=[
                [Button.inline("• Uᴘᴅᴀᴛᴇ Nᴏᴡ •", data="updatenow")],
                [Button.inline("« Bᴀᴄᴋ", data="ownr")],
            ],
        )
        remove("oreo_updates.txt")
    else:
        await event.edit(
            changelog_str,
            buttons=[
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Oreo Op", text=txt, link_preview=False, buttons=btn)
    ]
